...
const command: Command = {
    ...
    data: getScopedSlashCommandBuilder(CommandLangKey.UPLOAD_FILE_BASE_COMMAND_NAME, CommandLangKey.UPLOAD_FILE_BASE_COMMAND_DESCRIPTION)
        .addAttachmentOption((option) =>
            getScopedOption(option, CommandLangKey.UPLOAD_FILE_ATTACHMENT_NAME, CommandLangKey.UPLOAD_FILE_ATTACHMENT_DESCRIPTION)
                .setRequired(true)
        ).addStringOption((option) =>
            getScopedOption(option, CommandLangKey.UPLOAD_FILE_CATEGORY_NAME, CommandLangKey.UPLOAD_FILE_CATEGORY_DESCRIPTION)
                .setRequired(true)
                .setAutocomplete(true)
        ).addStringOption((option) =>
            getScopedOption(option, CommandLangKey.UPLOAD_FILE_FILE_NAME_NAME, CommandLangKey.UPLOAD_FILE_FILE_NAME_DESCRIPTION)
        )
        .toJSON(),
